...
import { Sparkles, Gift, Share2, Instagram, Users, Loader2, X, Download } from 'lucide-react';
import confetti from 'canvas-confetti';
import type { Rarity } from './types';
import { ATTEMPTS } from './helpers';

/* ─── Spark Canvas (intro bg) ────────────────────────────────────────── */
    ...
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-[#080012]">
      ...
      <div className="relative z-10 text-center px-6">
        ...
        <h2 className="text-4xl md:text-5xl font-bangers text-white leading-tight mb-4">
          🎁 Tienes <span className="text-[#FFB800]">{ATTEMPTS} intentos gratuitos</span><br />
          para rescatar a Chispín
        </h2>
        <p className="text-white/50 text-sm mb-8 font-nunito max-w-md mx-auto">
          Mueve el gancho, elige la profundidad y atrapa a tu Chispín. La máquina es 100% interactiva y real.
        </p>
        <button
          onClick={onStart}
...
function getRarityInfo(rarity: Rarity) {
  const map: Record<Rarity, { cls: string; txt: string }> = {
    normal: { cls: 'text-[#FFB800] border-[#FFB800]/30', txt: 'Edición Fundadores' },
    festivalero: { cls: 'text-purple-400 border-purple-400/30', txt: '⚡ Chispín Festivalero' },
    dorado: { cls: 'text-amber-300 border-amber-300/30', txt: '✨ Chispín Dorado — 1 de 500' },
    azul: { cls: 'text-blue-400 border-blue-400/30', txt: '💙 Fuego Azul — 1 de 1000' },
  };
  return map[rarity];
}

export function VictoryScreen({ founderId, rarity, onReserve, onClose }: VictoryScreenProps) {
  ...
  const downloadCert = () => {
    ...
    // Background
    const grad = ctx.createLinearGradient(0, 0, 0, 880);
    grad.addColorStop(0, '#1A0040');
    grad.addColorStop(0.5, '#2D0A5C');
    grad.addColorStop(1, '#080012');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, 620, 880);

    // Border
    ctx.strokeStyle = '#FFB800';
    ctx.lineWidth = 6;
    ctx.strokeRect(20, 20, 580, 840);

    // Inner border
    ctx.strokeStyle = 'rgba(255,184,0,0.3)';
    ctx.lineWidth = 2;
    ctx.strokeRect(35, 35, 550, 810);

    // Title
    ctx.fillStyle = '#FFB800';
    ctx.font = 'bold 52px Georgia, serif';
    ctx.textAlign = 'center';
    ctx.fillText('CHISPÍN', 310, 130);

    // Subtitle
    ctx.fillStyle = 'rgba(255,184,0,0.7)';
    ctx.font = '18px Georgia, serif';
    ctx.fillText('Edición Fundadores', 310, 165);

    // Certificate text
    ctx.fillStyle = '#C8A5FF';
    ctx.font = '22px Georgia, serif';
    ctx.fillText('Certificado de Fundador', 310, 280);

    ctx.fillStyle = 'rgba(255,255,255,0.6)';
    ctx.font = '16px Georgia, serif';
    ctx.fillText('Por la presente se otorga este certificado a', 310, 330);

    // Founder number
    ctx.fillStyle = '#FFB800';
    ctx.font = 'bold 72px Georgia, serif';
    ctx.fillText('#' + String(founderId).padStart(6, '0'), 310, 430);

    // Rarity
    const rLabels: Record<string, string> = {
      normal: 'Edición Fundadores',
      festivalero: 'Chispín Festivalero',
      dorado: 'Chispín Dorado — 1 de 500',
      azul: 'Fuego Azul — 1 de 1000',
    };
    ctx.fillStyle = 'rgba(200,165,255,0.8)';
    ctx.font = '16px Georgia, serif';
    ctx.fillText(rLabels[rarity] || '', 310, 480);

    // Date
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.font = '14px Georgia, serif';
    const now = new Date();
    ctx.fillText(now.toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }), 310, 540);

    // Footer
    ctx.fillStyle = 'rgba(255,184,0,0.3)';
    ctx.font = '12px Georgia, serif';
    ctx.fillText('chispin.es · La Chispa Nunca Se Apaga', 310, 820);

    c.toBlob((b) => {
      if (!b) return;
      const a = document.createElement('a');
      a.href = URL.createObjectURL(b);
      a.download = `certificado-chispin-${founderId}.png`;
      a.click();
    });
  };
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-[#080012]/95">
      <div className="text-center px-6 max-w-md">
        ...
        <h2 className="text-5xl font-bangers text-white mb-2">¡LO HAS RESCATADO!</h2>
        <p className="text-white/60 font-nunito text-sm mb-6">
          Tu Chispín está esperando. Completa tu reserva para convertirte en Fundador Oficial.
        </p>

        <div className="glass rounded-2xl p-6 mb-6 border border-[#4A0E8F]">
          <div className="text-3xl font-bangers text-[#FFB800] mb-1">
            #{String(founderId).padStart(6, '0')}
          </div>
        ...
        >
          RESERVAR MI CHISPÍN
          <span className="block text-xs font-nunito mt-1 text-[#1A0040]/70">39€ · Edición Fundadores</span>
        </button>

        <button
          onClick={downloadCert}
          className="mt-3 px-6 py-2.5 rounded-xl bg-white/5 border border-white/20 text-white/70 hover:text-white hover:bg-white/10 hover:border-white/40 transition-all font-nunito text-sm font-bold inline-flex items-center gap-2"
        >
...
export function ReserveScreen({ onBack, founderId }: ReserveScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({ name, email, founderId }),
      });
      ...
      <div className="glass rounded-3xl p-8 max-w-md w-full mx-4 border border-[#4A0E8F]">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <h3 className="font-bangers text-2xl text-white mt-2">Reserva tu Chispín</h3>
            <p className="text-white/60 text-sm">Edición Fundadores · 39€</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full text-center justify-center disabled:opacity-50 font-bangers">
            {loading ? <><Loader2 className="w-5 h-5 animate-spin inline-block mr-2" /> PROCESANDO...</> : 'RESERVAR POR 39€'}
          </button>
        </form>
        <p className="text-white/40 text-xs text-center mt-4 font-nunito">Pago seguro vía Stripe</p>
      </div>
    </div>
  );
}
...
export function NoAttemptsScreen({ onReserve, onEarnAttempt, onClose }: NoAttemptsScreenProps) {
  const socialActions = [
    { icon: <Share2 className="w-5 h-5" />, label: 'Comparte Chispín', reward: '+1 intento', action: () => onEarnAttempt(1) },
    { icon: <Instagram className="w-5 h-5" />, label: 'Síguenos en Instagram', reward: '+1 intento', action: () => onEarnAttempt(1) },
    ...
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-[#080012]/95 px-6">
      <div className="text-center max-w-md">
        ...
        <p className="text-white/50 font-nunito text-sm mb-8">
          La máquina es real. Si has llegado hasta aquí, sabes que el gancho, las cámaras y la profundidad no son un truco.
        </p>
